...
2. `http://dig.local:9778` (node)
3. `http://localhost:9778` (node)
4. `https://rpc.dig.net` (rpc) — the FINAL fallback.

- A node tier is selected only when a cheap `GET {base}/health` returns 2xx within a
  short timeout; otherwise the ladder falls through.
- An override is classified by its `/health`: healthy → node surface, else → rpc
  surface. An override MUST NOT silently fall back to the public gateway.
- The auto-ladder plan is `[node(first-healthy), rpc(rpc.dig.net)]` when a node tier
  is healthy, else `[rpc(rpc.dig.net)]`.
- The resolved plan SHOULD be cached per resolver instance.

## 5. Read paths

### 5.1 Node path (`EndpointKind::Node`)

`GET {base}/s/<store_id>[:<root>]/<resource_key>`.

- `2xx` → the body is the decrypted, node-verified plaintext (loopback trust). The
  content type is the response `Content-Type` header, else derived (§7).
- `404` → hard `NotFound`.
- other non-2xx / transport failure → treated as a transport failure (ladder falls
  through). No client-side crypto is performed on this path.

### 5.2 RPC path (`EndpointKind::Rpc`)

1. Resolve the trust root: the URN's pinned root, else `dig.getAnchoredRoot
   {store_id} -> {root}`. A missing/empty root → hard `NotFound`.
...
- `ResolveOutcome::Unreachable` — every tier was transport-unreachable; nothing was
  fetched.
- `Err(ResolveError)` — `Parse`, `NotFound`, or `Rpc` (a reachable protocol error).
...
- `resolve(urn, endpoint?, connectUrl?) : Promise<{ outcome, bytes, contentType }>`,
  `outcome ∈ "success" | "integrity_failure" | "unreachable"`.
- `resolveObjectUrl(urn, endpoint?, connectUrl?) : Promise<string>` — a `blob:` URL;
  the security/unreachable page for a non-success outcome, never unverified bytes.
...
- A root-pinned URN MUST NOT call `dig.getAnchoredRoot`.
- Node-absent + a serving rpc gateway MUST resolve a valid resource to `Success`.
