...
// The Node entry is a thin CommonJS wrapper that injects Node's `fs` into the wasm
// module (enabling the `cachePath` disk cache under Node) and then re-exports the
// wasm-bindgen glue unchanged. The BROWSER entry never loads this, so `cachePath`
// stays a harmless no-op clientside — no `fs` import ever reaches a browser bundle.
writeFileSync(
  `${OUT}/node/index.cjs`,
  [
    "// Auto-generated by scripts/build-npm.mjs. The Node front door: wire Node's",
    "// `fs` into the wasm module so `cachePath` persists a real disk cache, then",
    "// re-export the wasm-bindgen API unchanged.",
    'const wasm = require("./dig_urn_resolver.js");',
    'try { wasm.__dig_set_node_fs(require("fs")); } catch (_) { /* fs unavailable — disk cache stays inert */ }',
    "module.exports = wasm;",
    "",
  ].join("\n"),
);
...
const pkg = {
  ...
  // Node resolves through the CJS wrapper (index.cjs) so `fs` is injected before use.
  main: "./node/index.cjs",
  ...
  sideEffects: [
    "./web/dig_urn_resolver.js",
    "./node/dig_urn_resolver.js",
    "./node/index.cjs",
  ],
};
